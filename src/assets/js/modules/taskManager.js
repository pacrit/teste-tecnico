export class TaskManager {
  ...
  async loadTasks() {
    try {
      ...
      // console.log("Tarefas carregadas:", response); <-- Usado como teste de return
  ...
  async saveTask() {
    try {
      ...
      // console.log("Salvando tarefa:", formData); <-- Usado como teste de validação de dados
  ...
  deleteTask(id) {
    ...
    if (task) {
      ...
      ).textContent = `Tem certeza que deseja excluir a tarefa "${task.title}"?`;
  ...
  async changeTaskStatus(id, newStatus) {
    try {
      ...
      if (response.success) {
        ...
        this.ui.showAlert(
          `Tarefa ${statusLabels[newStatus]} com sucesso!`,
          "success"
        );
        await this.loadTasks();
      } else {

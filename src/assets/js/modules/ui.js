export class TaskUI {
    ...
    renderTasks(tasks) {
        ...
        // Adicionar animação
        this.tasksContainer.querySelectorAll('.task-card').forEach((card, index) => {
            setTimeout(() => {
    ...
    createTaskCard(task) {
        const statusInfo = this.statusLabels[task.status];
        const formattedDate = this.formatDate(task.created_at);
        const updatedDate = task.updated_at !== task.created_at ? 
            this.formatDate(task.updated_at) : null;
        
        return `
            <div class="col-md-6 col-lg-4">
                <div class="card task-card status-${task.status}" data-task-id="${task.id}">
                    <div class="card-body position-relative">
                        <span class="badge ${statusInfo.class} task-status-badge">
                            ${statusInfo.label}
                        </span>
                        ...
                        <div class="d-flex justify-content-between align-items-center task-actions">
                            <div class="btn-group" role="group">
                                <button class="btn btn-outline-primary btn-sm" onclick="window.editTask(${task.id})">
                        ...
                        <div class="task-meta">
                            ...
                            ${updatedDate ? `<div><i class="fas fa-calendar-check me-1"></i>Atualizado: ${updatedDate}</div>` : ''}
                        </div>
                    </div>
                </div>
            </div>
